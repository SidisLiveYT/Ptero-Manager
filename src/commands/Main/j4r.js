...
module.exports = {
  ...
  async execute(bot, message, args) {
    ...
    if (!Host_Owner.includes(`${message.author.id}`) && !Host_Developer.includes(`${message.author.id}`)) {
      const Embed_Error = {
    ...
    if (args[0].toLowerCase().trim() === 'help') {
      const Embedm = {
    ...
    if (args[0].toLowerCase().trim() === 'setup') {
      ...
      message.channel.send(Embed_Build).catch((err) => message.author.send(`__**Missing Permissions in Server/Channels**__\n**${message.guild.name}**\n\n__**Missing Permissions**__\n${hash} Send Messages OR Manage Messages ${hash}`).catch((err) => { }));
      ...
      message.channel.send(EmbedQuestions).catch((err) => message.author.send(`__**Missing Permissions in Server/Channels**__\n**${message.guild.name}**\n\n__**Missing Permissions**__\n${hash} Send Messages OR Manage Messages ${hash}`).catch((err) => { }));
      message.channel.awaitMessages(filter, { max: 1, time: 2 * 60 * 1000, error: ['time'] })
        .then((collected) => {
          response = collected.first();
          if (response === undefined) {
