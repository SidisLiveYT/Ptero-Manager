const {
  NormalEmbedGen, ErrorEmbedGen, DM_ErrorEmbedGen, DM_NormalEmbedGen,
...
module.exports = {
  Register_Server_J4R(bot, Book, message) {
    const connection = bot.Connection;
    connection.query(`INSERT INTO J4R_Servers (GuildID,Name,Ram,Cpu,Disk,Swap) VALUES ('${Book[0]}','${Book[1]}','${Book[2]}','${Book[3]}','${Book[4]}','${Book[5]}')`, (err) => {
      if (err) {
        const Embed_Error = {
      ...
      connection.query('SELECT * FROM J4R_Servers', (err, result) => {
        if (err) {
          const Embed_Error = {
            title: 'Error is Detcted in Database Gateway',
            des: '**Please check for Database Issues or Database Conenction Issues**',
  ...
  Delete_Server_J4R(bot, Book, message) {
    const connection = bot.Connection;
    connection.query(`DELETE FROM J4R_Servers WHERE GuildID = '${Book}'`, (err) => {
      if (err) {
        const Embed_Error = {
          title: 'Error is Detcted in Database Gateway',
          des: '**Please check for Database Issues or Database Conenction Issues**',
  ...
  Edit_Server_J4R(bot, Book, message) {
    const connection = bot.Connection;
    connection.query(`UPDATE J4R_Servers SET ${Book.Type} = '${Book.Value}' WHERE GuildID = '${Book.GuildID}'`, (err) => {
      if (err) {
        const Embed_Error = {
          title: 'Error is Detcted in Database Gateway',
          des: '**Please check for Database Issues or Database Conenction Issues**',
  ...
  Register_User_J4R(bot, Book, member) {
    const connection = bot.Connection;
    connection.query(`INSERT INTO J4R_Users_Journals (UserID,Username,GuildID,Guild_Name) VALUES ('${Book.UserID}','${Book.Name}','${Book.GuildID}','${Book.Guild_Name}')`, (err) => {
      if (err) { console.error(`Error is found :${err}`); }
      connection.query('SELECT * FROM J4R_Users_Journals', (err, result) => {
        if (err) { console.error(`Error is found :${err}`); }
  ...
  Remove_User_J4R(bot, Book, member) {
    const connection = bot.Connection;
    connection.query(`DELETE FROM J4R_Users_Journals WHERE UserID = '${Book.UserID}'`, (err) => {
      if (err) { console.error(`Error is found :${err}`); }
      connection.query('SELECT * FROM J4R_Users_Journals', (err, result) => {
        if (err) { console.error(`Error is found :${err}`); }
        bot.j4r_Journals_DB = result;
