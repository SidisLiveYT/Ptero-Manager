...
const { DM_ErrorEmbedGen,DM_NormalEmbedGen,ErrorEmbedGen,NormalEmbedGen } = require('./util/Embed_Generator.js');
...
bot.on('message', message => {
    ...
    let prefix_guild = process.env.Prefix;
...
bot.on('guildMemberAdd', member => {
  return Checker_Add_User_J4R(bot,member);
});

bot.on('guildMemberRemove', member => {
  return Checker_Remove_User_J4R(bot,member);
})
